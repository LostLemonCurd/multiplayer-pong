...
export function Ball(options) {
  ...
  this.update = (delta) => {
    this.position = Vector2.add(
      this.position,
      Vector2.multiply(dir, this.speed * delta)
    );
    ...
    // Right paddle
    if (
      this.position[0] + this.radius > options.rightPaddle.position[0] &&
      this.position[1] >= options.rightPaddle.position[1] &&
      this.position[1] <=
        options.rightPaddle.position[1] + options.rightPaddle.height
    ) {
      // Hit right paddle
      dir = randomHorReflect(dir, -1);
    } else if (
      this.position[0] - this.radius <
        options.leftPaddle.position[0] + options.leftPaddle.width &&
      this.position[1] >= options.leftPaddle.position[1] &&
      this.position[1] <=
        options.leftPaddle.position[1] + options.leftPaddle.height
    ) {
      // Hit left paddle
      dir = randomHorReflect(dir, 1);
    }

    // Modifiez cette ligne dans ball.js
